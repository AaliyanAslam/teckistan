...
export default function TeckistanNav() {
    ...
    const ctx = gsap.context(() => {
      gsap.timeline({ defaults: { ease: "power3.out" } })
        ...
        .fromTo(logoRef.current,
          { x: -20, opacity: 0 },
          { x: 0, opacity: 1, duration: 0.5 }, "-=0.35"
        )
        .fromTo(linksRef.current,
          { y: -14, opacity: 0 },
          { y: 0, opacity: 1, stagger: 0.07, duration: 0.4 }, "-=0.3"
        )
        .fromTo(ctaRef.current,
          { scale: 0.82, opacity: 0 },
          { scale: 1, opacity: 1, duration: 0.42, ease: "back.out(1.8)" }, "-=0.22"
        );
    });
    return () => ctx.revert();
  }, []);
  ...
  useEffect(() => {
    ...
    if (menuOpen) {
      ...
      gsap.fromTo(drawerItems.current,
        { x: -22, opacity: 0 },
        { x: 0, opacity: 1, stagger: 0.06, duration: 0.3, delay: 0.2, ease: "power2.out" }
      );
      gsap.fromTo(drawerFootRef.current,
        { y: 16, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.32, delay: 0.42, ease: "power2.out" }
      );
  ...
  // Link hover lift
  const hoverLink = (i, dir) =>
    gsap.to(linksRef.current[i], { y: dir === "in" ? -2 : 0, duration: 0.16, ease: "power2.out" });

  // CTA hover scale
  const hoverCta = (dir) =>
    gsap.to(ctaRef.current, { scale: dir === "in" ? 1.05 : 1, duration: 0.18, ease: "power2.out" });
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { font-family: 'Urbanist', sans-serif; box-sizing: border-box; }
        ...
        /* Active & hover dot under nav links */
        .link-dot {
          position: absolute;
          bottom: 1px; left: 50%;
          translate: -50% 0;
        ...
        .drawer-row.is-active .drawer-icon { background: #036083; }
        .drawer-row.is-active .drawer-icon svg { color: #fff !important; }
        ...
        <div className="max-w-[1260px] mx-auto h-full px-5 sm:px-8 flex items-center justify-between gap-6">
          ...
          <a ref={logoRef} href="#home" className="flex items-center gap-3 shrink-0">
            <div className="relative flex items-center justify-center w-[38px] h-[38px] rounded-[10px] shrink-0"
              ...
              <Zap size={17} color="#fff" fill="#fff" className="relative z-10" />
            </div>
            <div className="leading-none">
              <div className="font-black text-[16.5px] text-gray-900" style={{ letterSpacing: "-0.4px" }}>
                Teckistan
              </div>
              <div className="font-[650] text-[10px] tracking-[0.14em] uppercase" style={{ color: "#036083", marginTop: 1 }}>
                Solutions
              </div>
            </div>
          </a>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: "rgba(3,96,131,.09)" }}>
          <a href="#home" onClick={() => setMenuOpen(false)} className="flex items-center gap-2.5">
            ...
            <div className="leading-none">
              ...
              <div className="font-[650] text-[9.5px] tracking-[0.13em] uppercase" style={{ color: "#036083", marginTop: 1 }}>Solutions</div>
            </div>
          </a>
          <button
            onClick={() => setMenuOpen(false)}
          ...
          {NAV_LINKS.map(({ label, href, icon: Icon }, i) => (
            <li key={label} ref={el => drawerItems.current[i] = el}>
              ...
              >
                ...
                <span className="font-[650] text-[15px]">{label}</span>
